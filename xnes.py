...
credit:
https://github.com/chanshing/xnes/blob/master/xnes.py
"""
...
import scipy as sp
from numpy import (dot, eye,  asarray, array, trace, log, exp, sqrt, mean, sum, argsort, square, arange)
...
class XNES(object):
    def __init__(self, f, mu, amat,
        ...
        # default population size and learning rates in the paper
        ...
        for j in range(1, int(npop)+1):
            denom += max(0, np.log(npop/2. +1 ) - np.log(j))
        for i in range(1, dim):
            mu[i-1] =max(0, np.log(npop/2.+1 ) - np.log(i)) /denom - 1/ npop
        self.mu = mu
        '''
    ...
    def step(self, niter):
        ...
        for i in range(niter):
                ...
                if not  self.fitness_best:
                    self.fitness_best = fitness
                    self.mu_best = mu.copy()
                    self.counter = 0
               # elif fitness - 1e-8 > self.fitness_best:
                   # self.fitness_best = fitness
                    #self.mu_best = mu.copy()
                    #self.counter = 0
                else: 
